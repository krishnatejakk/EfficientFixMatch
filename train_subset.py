...
def get_cosine_schedule_with_warmup(optimizer,
    ...
    def _lr_lambda(current_step):
        ...
        no_progress = float((current_step%num_training_steps) - num_warmup_steps) / \
...
def main():
    ...
    optimizer1 = optim.SGD(grouped_parameters, lr=args.lr,
                          momentum=0.9, nesterov=args.nesterov)
    ...
    model.zero_grad()
    train_subset(args, labeled_dataset, unlabeled_dataset, test_dataset,
          model, model1, optimizer, optimizer1, ema_model, scheduler)


def train_subset(args, labeled_dataset, unlabeled_dataset, test_dataset,
          model, model1, optimizer, optimizer1, ema_model, scheduler):
    if args.dss_strategy == 'Full':
    ...
    unlabeled_seq_loader = DataLoader(
        unlabeled_dataset,
